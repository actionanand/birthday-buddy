...
The native contact plugin reads only lookup key, display name, thumbnail, favorite status, birthdays, anniversaries and custom contact events. Recognized labels such as betrothal, engagement, work/joining, friendship, first meet/relationship, memorial and wedding are mapped to their matching Birthday Buddy occasion; unknown custom labels remain custom occasions. Imported thumbnails are returned as app-owned image data and persisted with the person. Sync is preview-first and the application database remains the source of truth. Identical event rows exposed by local and Google raw contacts under one aggregated Android contact are collapsed before import. If Android keeps them as separate contacts, the user can merge them safely; every Android lookup key is retained on the merged record so later syncs continue updating the same person without recreating the duplicates.
